...
const CATEGORIES: Link["category"][] = ["face_care", "sport", "clothing", "tools"];

const TITLES: Partial<Record<Link["category"], string[]>> = {
  face_care: [
  ...
  shoes: ["Кроссовки беговые", "Кожаные дерби", "Ботинки на меху", "Балетки", "Сандалии"],
  accessories: ["Кожаный ремень", "Солнцезащитные очки", "Шёлковый платок", "Кошелёк", "Шапка"],
  home_decor: ["Плед из шерсти", "Керамическая кружка", "Аромасвеча", "Постельное бельё", "Ваза"],
  kids: ["Конструктор", "Развивающая книга", "Пижама детская", "Рюкзак школьный", "Пазл"],
  electronics: ["Беспроводные наушники", "Пауэрбанк", "Умная колонка", "Фитнес-браслет"],
  sport: [
    "Коврик для йоги", "Гантели", "Компрессионные гетры", "Бутылка для воды",
    "Рюкзак на 30 литров", "Складной нож для кемпинга", "Фонарь налобный", "Спортивная сумка",
  ],
  health: ["Витамин D3", "Омега-3", "Магний", "Протеиновый батончик"],
  books_stationery: ["Ежедневник", "Набор ручек", "Роман в мягкой обложке", "Блокнот"],
};
  ...
  await pool(Array.from({ length: CREATORS }, (_, i) => i), 20, async (i) => {
    ...
    for (let j = 0; j < linkCount; j++) {
      ...
      const link = await addLink({
        ...
        title: pick(TITLES[category] ?? ["Товар"]),
        category,
